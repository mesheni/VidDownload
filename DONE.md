...
- [x] 2.1 — **Внедрить MVVM: выделить `MainViewModel` и `ConvertViewModel`**
...
- [x] 3.3 — **История загрузок**: список последних ссылок с повторным скачиванием (реализовано в рамках 2.9)
...
- [x] 4.1 — **Миграция на .NET 10**: `net10.0-windows` вместо `net6.0-windows10.0.22621.0`
...
- [x] 7.2 — **Переключение языка в рантайме**: через `CultureInfo` или `ResourceDictionary`
  ...
  - [x] 7.2.9 — Обработать трюковой случай: окна, открытые в момент смены языка — обновить строки
  ...
  - [x] 7.2.12 — Функциональный тест: переключить язык во время загрузки/простоя — все строки обновляются без перезапуска
